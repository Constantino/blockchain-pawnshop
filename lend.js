...
async function getNFTsToLend(){  
    ...
    for (const index in information) {
      ...
      let contractName="PawnShop";//information[index].token_address
      let symbol="CAI";//information[index].symbol
        // here you can use the result of promise
        //data= information[index].attributes.token_uri;
              if(dataxImage !=null){
                //If NFT have image
                ele.innerHTML +='<div class="col"> <div class="card shadow-sm"> <img width="100%" height="100%" alt="NFT" src=" '+  dataxImage+'"/> <div class="card-body"> <p class="card-text">Token Id:' +  information[index].tokenId+'</p><p class="card-text">Contract Name:' + contractName +'</p><p class="card-text">Symbol:'+  symbol+'</p><p class="card-text">dailyInterestRate:' + dailyInterestRate +'</p><p class="card-text">Debt Term:' + debtTerm +'</p><p class="card-text">Closing Time:' + closingTime +'</p><div class="d-flex justify-content-between align-items-center"> <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal" onclick="enable(\'' +information[index].token_address + '\',\''+ information[index].contract_type +" \ '," +information[index].tokenId +","+ information[index].amount +"," + information[index].chunkPrice+"," + information[index].openingTime +","+ information[index].closingTime+","+ information[index].id +' ) ">Lend</button> </div></div></div></div>';
                //To-Do Decode JSON to obtaing image
              }
              else
              {
                ele.innerHTML +='<div class="col"> <div class="card shadow-sm"> <canvas width="100%" height="100%" style="border: 1px solid; background-color: #007ad5;"> </canvas> <div class="card-body"> <p class="card-text">Token Id:' + information[index].tokenId+'</p><p class="card-text">Contract Name:' + contractName+'</p><p class="card-text">Symbol:' +symbol +'</p><p class="card-text">dailyInterestRate:' + dailyInterestRate  +'</p><p class="card-text">Debt Term:' + debtTerm +'</p><p class="card-text">Closing Time:' + closingTime +'</p><div class="d-flex justify-content-between align-items-center"> <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal" onclick="enable(\'' + information[index].token_address + '\',\''+ information[index].contract_type +" \ '," +information[index].attributes.tokenId +"," + information[index].amount +"," + information[index].chunkPrice+"," + information[index].openingTime +","+ information[index].closingTime+","+ information[index].id +' ) ">Lend</button> </div></div></div></div>';
              }
    }
  }
